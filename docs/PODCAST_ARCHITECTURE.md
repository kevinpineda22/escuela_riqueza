...
| La lección se repetía sola al completarse | Al terminar una lección en modo podcast, el audio volvía a empezar desde 0 en vez de detenerse | **`pause` se dispara ANTES que `ended`.** `handlePause` evaluaba `endedRef.current` (todavía `false`) y programaba un `play()` a 200ms; para cuando ese timeout corría la pista ya había terminado, y `play()` sobre una pista terminada la reinicia desde cero. El watchdog podía hacer lo mismo. | Revalidar **dentro** del `setTimeout` de `handlePause`, no solo al entrar. Se agregó `isAtEnd(el)` (`currentTime >= duration - 0.5`) como red de seguridad independiente del evento `ended`, aplicada en `handlePause`, el watchdog, `handleCanPlay` y el listener de `visibilitychange`. Estado `hasEnded` en el store para que la UI muestre "Lección completada". |
...
6. **No persisitir `isPlaying` en el store.** Al recargar la página, el navegador bloquea autoplay. El estado de reproducción debe comenzar como `false` y activarse con gesto del usuario. Lo mismo aplica a `hasEnded`: es efímero.
7. **Todo `play()` diferido debe revalidar el estado en el momento de ejecutarse**, no solo al programarse. El orden de eventos al terminar es `pause` → `ended`, así que cualquier guard evaluado antes de un `setTimeout` mira un estado que va a quedar obsoleto. Usar `endedRef.current || isAtEnd(el)` dentro del callback.
