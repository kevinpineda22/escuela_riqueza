...
- **Modo "Baja latencia"**: usa LL-HLS de Cloudflare Stream (`?protocol=llhls` en el manifest, **beta** de Cloudflare) — `lowLatencyMode: true` en hls.js, sin `liveSyncDuration` fijo (se deriva de `PART-HOLD-BACK`). Esperado ~2-4s del edge. OBS: keyframe 1-2s, B-frames desactivados (requisito de Cloudflare para LL-HLS). "Fluidez" sigue siendo HLS estándar sin el flag y es el fallback si LL-HLS falla en un live real. WHIP/WebRTC solo si algún día se necesita sub-segundo. Ver CHANGELOG 2026-09-21.
