...
- **Reacciones al chat**: tabla `live_message_reactions` (PK `message_id, user_id, emoji`, 4 keys fijas: `heart`, `fire`, `clap`, `raised_hands`), mismo canal Realtime que los mensajes; el link público las lee agregadas (sin identidad) vía `get_public_live_reactions`.
- **Tres modos de latencia** (`LiveLatencyMode`, selector en `LivePlayerControls`): "Fluidez" (`smooth`, default) — HLS estándar, ~8-10s del edge, buffer 20s, el fallback más robusto. "Baja latencia" (`low`) — LL-HLS de Cloudflare Stream (`?protocol=llhls`, **beta** de Cloudflare), `lowLatencyMode: true` en hls.js sin `liveSyncDuration` fijo (se deriva de `PART-HOLD-BACK`), ~2-4s del edge; OBS: keyframe 1-2s, B-frames desactivados (requisito de Cloudflare para LL-HLS). "Clase completa" (`dvr`) — DVR de Cloudflare Stream (`?dvrEnabled=true`), misma config hls.js que "Fluidez", ~14.6s de latencia medida pero con ventana seekable que crece con la transmisión (timeline scrubbable en los controles) y resume automático de posición vía `localStorage` (`live-position:<liveId>`). `?protocol=llhls` y `?dvrEnabled=true` son **mutuamente excluyentes** (combinarlos da HTTP 500). WHIP/WebRTC solo si algún día se necesita sub-segundo. Ver CHANGELOG 2026-09-21 y 2026-09-22.
- **Link público**: el admin puede marcar una sala `is_public` (`AdminLiveManager`) y compartir `/live/<share_token>`. El control de toggle/URL/copiar (`PublicLinkControl`, componente local en `AdminLiveManager.tsx`) está disponible tanto en salas activas como en "Finalizados", para poder compartir/reactivar el link de una repetición ya terminada. La ruta pública (`PublicLiveRoom`) no requiere login ni valida plan; el estado se obtiene solo por polling a `get_public_live` (RPC token-gated) y el chat es de solo lectura vía `get_public_live_messages` (con sesión iniciada, chat completo). Ver `sql/migrate-lives-public-link.sql`. **El en vivo es público, pero la repetición no — salvo opt-in por sala**: por defecto, al finalizar, solo Individual/VIP/admin (`canWatchReplay` en `src/lib/plans.ts`) ven el replay (sin chat ni presencia; R2 vía `/api/stream/recording-url` autenticado con `live_id`, Stream vía iframe); Free y anónimos ven un paywall. `get_public_live` anula `recording_stream_uid`/`recording_r2_key` para quien no está habilitado — ver `sql/migrate-public-live-replay-paid.sql`. Como esas columnas nunca llegan sin plan pago, el paywall usa `public_live_has_recording` (RPC booleana, no revela el id/key) para saber si hay grabación bloqueada — ver `sql/migrate-public-live-has-recording.sql`. El admin puede además abrir la repetición de UNA sala puntual a cualquiera sin cuenta ni plan con el toggle "Repetición abierta a todos" (columna `lives.replay_is_public`, default `false`, dentro de `PublicLinkControl`, solo visible con el link público activo) — `get_public_live` revela las columnas de grabación si hay entitlement de plan **o** `replay_is_public = true` (ver `sql/migrate-public-live-open-replay.sql`). La firma de la URL de R2 para ese caso pasa por la rama anónima de `/api/stream/recording-url` (`share_token`, sin JWT), que vuelve a validar `replay_is_public` server-side antes de firmar nada — `fetchPublicRecordingUrl` en `src/lib/api/stream/lives.ts` la consume con `fetch` crudo (no `authedFetch`, no hay JWT que mandar).
