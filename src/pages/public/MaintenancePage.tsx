...
const MaintenancePage = () => {
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-[#050505] light:bg-surface-page flex flex-col items-center justify-center relative overflow-hidden font-sans px-5 sm:px-6 py-12 selection:bg-brand/30">
      ...
      <motion.div
        aria-hidden
        className="hidden md:block absolute -top-1/3 -left-1/4 w-[80vw] h-[80vw] max-w-[900px] max-h-[900px] rounded-full bg-[radial-gradient(circle_at_center,rgba(204,164,59,0.22)_0%,rgba(204,164,59,0.03)_45%,transparent_70%)] blur-3xl light:opacity-50 pointer-events-none"
      ...
      >
        ...
        <div className="relative mb-6">
          <div
            aria-hidden
            className="absolute inset-0 rounded-full bg-brand/25 blur-2xl scale-150"
          />
          <motion.div
            initial={{ rotate: 0 }}
            animate={{ rotate: [0, 12, -12, 0] }}
            transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
            className="relative w-20 h-20 rounded-full bg-gradient-to-br from-brand/25 to-amber-500/10 border border-brand/40 flex items-center justify-center shadow-[0_0_30px_rgba(204,164,59,0.35)]"
          >
            <Wrench className="text-accent" size={34} strokeWidth={1.8} />
          </motion.div>
        </div>

        <div className="flex flex-col gap-3">
          <span className="inline-flex items-center justify-center gap-2 mx-auto px-3 py-1 rounded-full bg-brand/10 border border-brand/30 text-[10px] uppercase tracking-[0.3em] font-bold text-accent/90">
          ...
          <p className="text-sm sm:text-base text-foreground-muted leading-relaxed max-w-md mx-auto">
            {message}
          </p>
        </div>
