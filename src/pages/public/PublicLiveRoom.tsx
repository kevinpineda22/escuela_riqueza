...
const PublicLiveRoom = () => {
  ...
  return (
    <div className="h-[100dvh] bg-black text-textMain flex flex-col md:flex-row overflow-hidden font-sans">
      <div className={cn("flex flex-col relative", isDesktop ? "flex-1 md:h-screen" : mobileVideoHeightClass)}>
        {/* Header */}
        <motion.div
          initial={{ y: -100 }}
          animate={{ y: 0 }}
        ...
        >
          ...
          <div className="flex flex-col items-end gap-2 sm:gap-3 pointer-events-auto shrink-0">
            ...
            {!isEnded && viewerCount > 0 && (
              <div className="flex items-center gap-1.5 sm:gap-2 bg-black/50 backdrop-blur-md border border-gold/20 text-white/85 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black tracking-wide">
