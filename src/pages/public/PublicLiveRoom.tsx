...
const PublicLiveRoom = () => {
  ...
  return (
    <div className="h-[100dvh] bg-black text-textMain flex flex-col md:flex-row overflow-hidden font-sans">
      <div className={cn("flex flex-col relative", isReplay || isDesktop ? "flex-1 md:h-screen" : mobileVideoHeightClass)}>
        {/* Header */}
        <motion.div
          initial={{ y: -100 }}
          animate={{ y: 0 }}
        ...
        >
          ...
          <div className="flex flex-col items-end gap-2 sm:gap-3 pointer-events-auto shrink-0">
            {isEnded && hasRecording ? (
              <div className="flex items-center gap-1.5 sm:gap-2.5 bg-gold/15 backdrop-blur-md border border-gold/40 text-gold px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black tracking-widest">
            ...
            {showIframe && hasStreamId ? (
                ...
                {isR2Recording ? (
                  r2RecordingLoading ? (
                    ...
                    <div className="text-center p-8">
                      ...
                      <h2 className="text-2xl font-bold text-white mb-2">No pudimos cargar la grabación</h2>
                      <p className="text-textMuted max-w-md mx-auto">Probá recargar la página en unos minutos.</p>
                    </div>
                  )
                ) : CF_RECORDING_HOST ? (
                  <iframe
              ...
              <motion.div key="no-stream" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-full flex items-center justify-center z-10">
                <div className="text-white text-center p-12 bg-darker rounded-3xl border border-white/5">
                  ...
                  <p className="text-textMuted max-w-sm mx-auto">Esta sala todavía no tiene una señal vinculada.</p>
                </div>
              </motion.div>
            ) : (
              <motion.div
        ...
        {isDesktop && !isReplay && (
          <button
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 z-40 flex items-center gap-2 pl-3 pr-2.5 py-4 rounded-l-2xl bg-black/60 backdrop-blur-md border border-r-0 border-white/15 text-white/80 hover:text-gold hover:bg-black/80 transition-all"
          >
