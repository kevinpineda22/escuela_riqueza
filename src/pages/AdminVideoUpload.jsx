...
import { UploadCloud, FileVideo, X, CheckCircle, Loader2, LogOut, LayoutDashboard, Video, Users, Settings } from 'lucide-react';
...
const AdminVideoUpload = () => {
  ...
  const handleLogout = () => {
    // Cerrar sesión mock - te lleva al login
    navigate('/login');
  };
  ...
  const handleSubmit = (e) => {
    ...
    // MOCK: Simulando subida asíncrona
    setTimeout(() => {
      setStatus('success');
      setTimeout(() => {
        setStatus('idle');
  ...
  return (
    <div className="min-h-screen bg-darker flex flex-col md:flex-row text-textMain font-sans">
      ...
      {/* Sidebar Admin */}
      <aside className="w-full md:w-64 bg-black/50 border-r border-white/5 flex flex-col pt-6 md:min-h-screen shrink-0 relative z-10 hidden md:flex">
        ...
        <nav className="flex-1 px-4 space-y-2">
          ...
          <button className="flex items-center gap-3 w-full px-4 py-3 bg-gold text-darker font-bold rounded-xl shadow-[0_0_10px_rgba(204,164,59,0.3)]">
            <UploadCloud size={20} /> Cargar Lección
          </button>
          <button className="flex items-center gap-3 w-full px-4 py-3 text-textMuted hover:text-white hover:bg-white/5 font-medium rounded-xl transition-all">
            <LayoutDashboard size={20} /> Metricas
          </button>
          <button className="flex items-center gap-3 w-full px-4 py-3 text-textMuted hover:text-white hover:bg-white/5 font-medium rounded-xl transition-all">
        ...
        <div className="p-4 mt-auto border-t border-white/5">
          <button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-3 text-red-400 hover:bg-red-500/10 font-medium rounded-xl transition-all">
      ...
      <main className="flex-1 p-6 md:p-10 lg:p-16 relative overflow-y-auto">
        ...
        <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-3 px-4 py-2 bg-white/5 rounded-full border border-white/10 shrink-0">
             <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-gold to-yellow-600 border border-gold flex items-center justify-center text-xs font-bold text-darker">AD</div>
        ...
        {/* Zona del Formulario (Tarjeta) */}
        <div className="bg-white/[0.02] border border-white/[0.05] backdrop-blur-xl rounded-3xl p-6 lg:p-10 shadow-2xl">
          <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row gap-10">
            
            {/* Columna Izquierda: Datos del Video */}
            <div className="flex-1 space-y-6">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-textMuted uppercase tracking-wider">Título de la Lección</label>
                <input
                  type="text"
                  name="title"
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-textMuted uppercase tracking-wider">Módulo Perteneciente</label>
                <div className="relative">
                  <select
                    ...
                    required
                  >
                    <option value="" disabled className="bg-darker">Selecciona un Módulo Oficial</option>
                    <option value="aprendizaje" className="bg-darker">Inteligencia del aprendizaje</option>
                    <option value="riqueza" className="bg-darker">Inteligencia de la riqueza</option>
                    <option value="emocional" className="bg-darker">Inteligencia emocional</option>
                    <option value="comercial" className="bg-darker">Inteligencia Comercial y Negociadora</option>
                    <option value="estrategica" className="bg-darker">Inteligencia Estratégica</option>
                    <option value="espiritual" className="bg-darker">Inteligencia Espiritual</option>
                  </select>
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-textMuted uppercase tracking-wider">Descripción y Notas Resumen</label>
                <textarea
                  name="description"
            ...
            {/* Columna Derecha: Subida del Archivo */}
            <div className="w-full lg:w-[400px] shrink-0">
              <label className="text-sm font-semibold text-textMuted uppercase tracking-wider block mb-2">Archivo Multimedia (.MP4)</label>
              ...
              <div
                ...
                className={`relative w-full h-[300px] border-2 border-dashed rounded-2xl flex flex-col items-center justify-center transition-all ${
                ...
                {videoFile ? (
                  <div className="flex flex-col items-center justify-center text-center p-6">
                    <div className="w-16 h-16 bg-gold/20 text-gold rounded-full flex items-center justify-center mb-4 relative">
                       <FileVideo size={30} />
                       {status === 'success' && <CheckCircle className="absolute -bottom-1 -right-1 text-green-500 bg-darker rounded-full" size={20} />}
                    </div>
                    ...
                    {status === 'idle' && (
                      <button type="button" onClick={(e) => { e.stopPropagation(); setVideoFile(null); }} className="mt-4 text-xs font-semibold px-4 py-2 bg-white/10 hover:bg-red-500/20 hover:text-red-400 rounded-lg transition-colors">
                        Reemplazar Archivo
                      </button>
                    )}
                  </div>
                  ...
                  <div className="text-center p-6">
                    ...
                    <p className="text-sm text-textMuted mt-2 px-4">El tamaño máximo recomendado por Cloudflare Stream es de 5GB por archivo.</p>
                  </div>
                )}
              ...
              >
                ...
                {status === 'success' && <><CheckCircle size={20} /> ¡Publicado con éxito!</>}
              </button>
            </div>
