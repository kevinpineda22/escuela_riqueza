...
const Toggle = ({
  ...
  <div
    className={cn(
      "flex items-start justify-between gap-4 rounded-xl border border-line-subtle bg-ink/[0.04] p-4 transition-colors light:bg-surface-panel light:shadow-panel",
      !disabled && "hover:bg-ink/[0.06]",
      disabled && "opacity-60"
    )}
  >
    <div className="min-w-0 pr-2">
      <div className="flex items-center gap-2">
        <span className="text-sm font-semibold text-foreground-strong">{label}</span>
        {badge && (
          <span className="rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-amber-300 light:text-warning ring-1 ring-amber-500/30">
            {badge}
          </span>
        )}
      </div>
    ...
    <button
      ...
      disabled={disabled}
      className={cn(
        "relative mt-1 inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-focus/40",
    ...
    >
      <span
        className={cn(
          ...
          enabled ? "translate-x-5 bg-surface-page" : "translate-x-0 bg-ink/80"
        )}
      />
    </button>
  </div>
...
const Field = ({ label, value, onChange, placeholder, type = "text", icon: Icon, hint, prefix, min, max, step }: FieldProps) => (
  <div>
    <label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-foreground-muted">{label}</label>
    <div
      className={cn(
        "group flex items-center overflow-hidden rounded-xl border border-line-subtle bg-black/40 transition-colors focus-within:border-brand/50 focus-within:ring-2 focus-within:ring-focus/15 light:bg-surface-panel",
        Icon || prefix ? "pl-3" : ""
      )}
    >
      {Icon && <Icon size={15} className="shrink-0 text-foreground-muted group-focus-within:text-accent" />}
      {prefix && <span className="shrink-0 pr-2 text-sm text-foreground-muted">{prefix}</span>}
      <input
        type={type}
        value={value}
        ...
        max={max}
        step={step}
        className="w-full bg-transparent px-3 py-3 text-sm text-foreground-strong placeholder:text-foreground-muted/60 focus:outline-none"
      />
    </div>
    {hint && <p className="mt-1.5 text-[11px] text-foreground-muted/70">{hint}</p>}
  </div>
);
...
const Section = ({ title, description, children, icon: Icon }: { title: string; description?: string; children: React.ReactNode; icon?: React.ElementType }) => (
  <div>
    <div className="mb-4 flex items-start gap-3">
      {Icon && (
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand/15 text-accent ring-1 ring-focus/30">
          <Icon size={16} />
        </div>
      )}
      <div>
        <h3 className="text-lg font-bold text-foreground-strong">{title}</h3>
...
const Banner = ({
  ...
  const styles = {
    info: "bg-blue-500/5 border-blue-500/20 text-blue-300 light:text-info",
    warn: "bg-amber-500/5 border-amber-500/20 text-amber-300 light:text-warning",
    preview: "bg-purple-500/5 border-purple-500/20 text-purple-300 light:text-violet",
  }[variant];
  return (
    <div className={cn("flex items-start gap-3 rounded-xl border p-4", styles)}>
      ...
      <div className="min-w-0">
        ...
        <div className="mt-1 text-xs text-foreground-muted">{children}</div>
      </div>
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="mb-6 flex shrink-0 flex-col items-start justify-between gap-4 sm:mb-8 sm:flex-row sm:items-end">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-extrabold tracking-tight text-foreground-strong sm:gap-3 sm:text-3xl">
            <Settings className="text-accent" size={28} /> Ajustes del Sistema
          </h1>
        ...
        <div className="flex w-full items-center gap-2 sm:w-auto sm:gap-3">
          <AnimatePresence>
            {dirty && !saving && (
              <motion.button
                ...
                exit={{ opacity: 0 }}
                onClick={handleReset}
                className="rounded-xl px-3 py-2.5 text-sm text-foreground-muted transition-colors hover:bg-ink/5 hover:text-foreground-strong"
              >
                Descartar
              </motion.button>
            )}
          ...
          <button
            ...
            className={cn(
              ...
              dirty
                ? "bg-brand text-on-brand shadow-[0_4px_14px_rgba(204,164,59,0.4)] hover:bg-brand-hover"
                : "cursor-not-allowed bg-ink/5 text-foreground-muted"
            )}
          >
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 md:flex-row md:gap-8">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "border border-brand/30 bg-gradient-to-r from-brand/15 to-transparent text-foreground-strong shadow-[0_4px_18px_-8px_rgba(204,164,59,0.4)]"
                    : "border border-transparent text-foreground-muted hover:bg-ink/5 hover:text-foreground-strong"
                )}
              >
                <div className="flex items-center gap-2 md:gap-3">
                  <Icon size={16} className={isActive ? "text-accent" : "opacity-70"} />
                  <span>{tab.label}</span>
                </div>
                <div className="mt-0.5 hidden pl-7 text-[11px] text-foreground-muted/70 md:block">{tab.hint}</div>
              </button>
            );
          })}
        </aside>
        ...
        {/* Content */}
        <div className="custom-scrollbar relative flex-1 overflow-y-auto rounded-2xl border border-line-subtle bg-black/30 p-5 sm:p-6 light:bg-surface-panel light:shadow-panel">
          <AnimatePresence mode="wait">
            ...
            {activeTab === "brand" && (
              ...
              >
                ...
                <Section title="Logotipo" description="Sube el logo de tu plataforma. Se sirve por CDN de Cloudflare Images." icon={Palette}>
                  <LogoUploader
              ...
              >
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            active
                              ? "border-accent bg-gradient-to-br from-brand/15 to-transparent text-foreground-strong shadow-[0_0_20px_-8px_rgba(204,164,59,0.5)]"
                ...
                <Section title="Plan FREE (Anuncios)" description="Configuración de la publicidad para usuarios gratuitos." icon={Megaphone}>
                  <div className="space-y-6">
                    {/* Modo de aparición */}
                    <div>
                      <label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-foreground-muted">Modo de aparición</label>
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                ...
                                active
                                  ? type === "none"
                                    ? "border-red-500/50 bg-red-500/10 text-danger shadow-[0_0_12px_rgba(239,68,68,0.2)]"
                                    : "border-accent bg-gradient-to-r from-brand/15 to-transparent text-foreground-strong shadow-[0_0_12px_rgba(204,164,59,0.3)]"
                                  : "border-line-subtle bg-ink/[0.03] light:bg-surface-panel text-foreground-muted hover:border-ink/25 hover:text-foreground-strong"
                              )}
                            >
                              {labels[type]}
            ...
            {activeTab === "payments" && (
              ...
              >
                ...
                <Section title="Configuración de cobros" description="Moneda, periodo de prueba y precios mensuales." icon={DollarSign}>
                  <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                      <label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-foreground-muted">Moneda</label>
                        ...
                        {(["USD", "COP", "MXN", "EUR"] as const).map((c) => (
                          <button
                            ...
                            className={cn(
                              ...
                              draft.currency === c
                                ? "border-accent bg-brand/15 text-accent"
                                : "border-line-subtle bg-ink/[0.03] light:bg-surface-panel text-foreground-muted hover:border-ink/25 hover:text-foreground-strong"
                            )}
                          >
                            {c}
                ...
                <Section title="Planes" description="Precio mensual por plan." icon={CreditCard}>
                  <div className="space-y-4">
                    <div className="rounded-2xl border border-line-subtle bg-gradient-to-br from-white/[0.04] to-transparent p-5">
                      <div className="mb-3 flex items-center justify-between">
                        <div>
                          <div className="text-xs font-bold uppercase tracking-wider text-foreground-muted">Plan Individual</div>
                          <div className="mt-0.5 text-lg font-bold text-foreground-strong">{fmtPrice(draft.price_individual_monthly)}<span className="text-sm font-normal text-foreground-muted">/mes</span></div>
                        </div>
                        <span className="rounded-full bg-ink/10 px-2 py-0.5 text-[10px] font-semibold text-fg-80">Estándar</span>
                      </div>
                      <Field
                    ...
                    <div className="rounded-2xl border border-brand/30 bg-gradient-to-br from-gold/[0.08] to-transparent p-5 shadow-[0_0_24px_-12px_rgba(204,164,59,0.4)]">
                      <div className="mb-3 flex items-center justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-bold uppercase tracking-wider text-accent">Plan VIP</span>
                            <Sparkles size={12} className="text-accent" />
                          </div>
                          <div className="mt-0.5 text-lg font-bold text-foreground-strong">{fmtPrice(draft.price_vip_monthly)}<span className="text-sm font-normal text-foreground-muted">/mes</span></div>
                        </div>
                        <span className="rounded-full bg-brand/20 px-2 py-0.5 text-[10px] font-bold text-accent">Premium</span>
                      </div>
                      <Field
                        label="Precio mensual"
                ...
                <Section title="Procesador de pagos" description="Stripe (pendiente de integración)." icon={CreditCard}>
                  <div className="rounded-2xl border border-dashed border-ink/15 bg-ink/[0.02] p-5 light:bg-surface-panel/60">
                    <div className="flex items-start gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#635BFF]/10 text-[#635BFF]">
                        <CreditCard size={18} />
                      </div>
                      <div className="flex-1">
                        <div className="font-bold text-foreground-strong">Stripe</div>
                        <p className="mt-0.5 text-xs text-foreground-muted">Las llaves API se guardarán como variables de entorno seguras (server-side). Sólo se mostrarán los últimos 4 caracteres una vez configuradas.</p>
                      </div>
                      <span className="rounded-full bg-amber-500/15 px-2 py-1 text-[10px] font-bold uppercase text-amber-300 light:text-warning ring-1 ring-amber-500/30">No conectado</span>
                    </div>
                  </div>
                </Section>
              </motion.div>
            ...
            {activeTab === "notifications" && (
              ...
              >
                ...
                <Section title="Plantilla de confirmación de cuenta" description="Diseño actual del correo que envía Supabase al registrarse." icon={Sparkles}>
                  <div className="rounded-2xl border border-line-subtle bg-ink/[0.02] p-5 text-sm text-foreground-muted light:bg-surface-panel light:shadow-panel">
                    <p>
                      La plantilla branded está en{" "}
                      <code className="rounded bg-ink/10 px-1.5 py-0.5 text-xs text-foreground-strong">docs/email-templates/confirm-signup.html</code>.
                    </p>
                    <p className="mt-2">
                      Para aplicarla: <strong className="text-foreground-strong">Supabase Dashboard → Authentication → Email Templates → "Confirm signup"</strong> y pegar el contenido HTML.
                    </p>
                    <p className="mt-2">
                      Asunto sugerido: <em className="text-fg-80">"Confirma tu cuenta en {draft.platform_name}"</em>.
                    </p>
                  </div>
                </Section>
              </motion.div>
