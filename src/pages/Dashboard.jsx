...
import { BookOpen, Target, Heart, Briefcase, Map, Lightbulb, PlayCircle, Trophy, Users, Edit3, Award } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const [selectedModule, setSelectedModule] = useState(2); // Por defecto inteligencia de la riqueza
  const [activeLesson, setActiveLesson] = useState(lecciones[0]);
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside className="col-span-1 border-r border-white/10 pr-6 lg:block hidden">
          ...
          <nav className="space-y-2">
            {[
              { id: 'modulos', lbl: 'Módulos y Clases', icon: <PlayCircle size={18} /> },
              { id: 'notas', lbl: 'Notas Personales', icon: <Edit3 size={18} /> },
              { id: 'certificados', lbl: 'Certificados', icon: <Award size={18} /> },
              { id: 'comunidad', lbl: 'Comunidad VIP', icon: <Users size={18} /> },
            ].map(tab => (
              <button 
                key={tab.id}
        ...
        <div className="col-span-1 lg:col-span-3">
          {activeTab === 'modulos' && (
            <>
              {/* Carrusel de módulos rápidos */}
              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide mb-8">
                {modulosData.map(mod => (
                  <button 
                    key={mod.id}
                    onClick={() => setSelectedModule(mod.id)}
                    className={`flex-shrink-0 flex items-center gap-3 px-5 py-3 border rounded-full transition-all whitespace-nowrap ${
              ...
              {/* View del reproductor */}
              {filteredLessons.length > 0 ? (
                <>
                  <div className="mb-6 flex justify-between items-end">
                    <div>
                      <h2 className="text-3xl font-bold text-white mb-2">{activeLesson.titulo}</h2>
                      {/* Aquí mostramos si hay progreso en esta lección */}
                      <span className="text-textMuted flex items-center gap-2 text-sm"><Trophy size={16} className="text-gold" /> Módulo actulizado. Tú eres Premium.</span>
                    </div>
                  </div>

                  <LessonPlayer videoSrc={activeLesson.video} isPremium={true} />

                  {/* Panel de notas (sólo en PREMIUM) */}
                  <div className="mt-8 bg-black/30 border border-white/10 rounded-2xl p-6">
                    <h4 className="text-white font-medium mb-3 flex items-center gap-2"><Edit3 size={18} /> Apuntes Rápidos</h4>
                    <textarea 
                      value={personalNote}
                    ...
                    <div className="flex justify-end mt-3">
                      <button className="px-5 py-2 text-sm bg-white/10 hover:bg-gold hover:text-darker border border-white/10 text-white font-medium rounded-lg transition-all">Guardar Notas</button>
                    </div>
                  </div>
                </>
              ) : (
          ...
          {activeTab === 'comunidad' && (
            <div className="p-6 text-center border border-transparent rounded-3xl bg-gradient-to-br from-gold/20 to-darker h-64 flex flex-col justify-center relative overflow-hidden">
              <div className="relative z-10">
                ...
                <p className="text-white/80 mt-2 max-w-md mx-auto">Conecta con cientos de líderes y haz networking del más alto nivel.</p>
                <button className="mt-6 px-6 py-2 bg-gold text-darker font-bold rounded-lg hover:scale-105 transition-transform shadow-lg">Abrir Foro</button>
              </div>
            </div>
          )}
