  ...
  if (variant === "labeled") {
    return (
      <button
        ...
        aria-label={aria}
        className={`inline-flex items-center gap-2 text-xs font-medium text-foreground-muted hover:text-accent transition-colors ${className}`}
      >
        ...
        >
          <span
            className={`h-3 w-3 rounded-full transition-transform ${
              animationsEnabled
                ? "translate-x-3 bg-brand"
                : "translate-x-0.5 bg-ink/50"
            }`}
          />
        </span>
      </button>
      ...
      <TooltipTrigger asChild>
        <button
          ...
          className={`inline-flex items-center justify-center h-9 w-9 rounded-full border transition-colors ${
            animationsEnabled
              ? "border-brand/30 text-accent/90 hover:text-accent hover:border-brand/50 bg-brand/5"
              : "border-ink/15 text-fg-60 hover:text-foreground-strong hover:border-ink/30 bg-ink/5"
          } ${className}`}
        >
